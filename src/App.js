...
function App() {
	...
	const loadBlockchainData = async () => {
		if (typeof window.ethereum !== 'undefined') {
			...
			const plazas = await land.methods.getPlazas().call()
			setPlazas(plazas)
	...
	const buyHandler = async (_id) => {
		try {
			...
			const plazas = await landContract.methods.getPlazas().call()
			setPlazas(plazas)
			...
			setLandName(plazas[_id - 1].name)
			setLandOwner(plazas[_id - 1].owner)
			setHasOwner(true)
		} catch (error) {
